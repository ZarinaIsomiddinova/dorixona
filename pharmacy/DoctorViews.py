...
def addPrescription(request,pk):        
    ...
    if request.method == 'POST':
        try:
            ...
            if form.is_valid():
                form.save()
                messages.success(request,'Retsept muvaffaqiyatli kiritildi')
                return redirect('manage_precrip_doctor')
        except:
            messages.error(request,'Retsept qo\'shilmagan')
...
def deletePrescription(request,pk):
    ...
    if request.method == 'POST':
        try:
            ...
            messages.error(request,'Retsept muvaffaqiyatli o\'chirilmadi')
            return redirect('manage_precrip_doctor')
    ...
    if request.method == 'POST':
        ...
        try:
            if form.is_valid():
                ...
                messages.success(request,'Retsept muvaffaqiyatli yangilandi')
                return redirect('manage_precrip_doctor')
        except:
            messages.error(request,'Xato!! Retsept yangilanmagan')
            return redirect('manage_precrip_doctor')
