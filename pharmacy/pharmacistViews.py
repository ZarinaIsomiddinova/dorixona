    ...
    try:
        ...
        if request.method == 'POST':
            if form.is_valid():
                ...
                messages.success(request, "Dori muvaffaqiyatli tarqatildi")

                return redirect('manage_patient_pharmacist')
            else:
                messages.error(request, "Yaroqlilik xatosi")
        ...
        messages.error(request,
                       "Tarqatishga ruxsat berilmagan! Dori muddati tugagan, qayta zaxiralash uchun administratorga murojaat qiling")
        return redirect('manage_patient_pharmacist')
    context = {
        "patients": queryset,
        "form": form,
...
def deletefeedback(request, pk):
    try:
        ...
        if request.method == 'POST':
            fed.delete()
            messages.success(request, "Fikr-mulohaza o‘chirib tashlandi")
            return redirect('patient_feedback_message')

    except:
        messages.error(request, "Fikr-mulohaza xatosi, qayta tekshiring")
        return redirect('patient_feedback_message')
...
def deleteDispense4(request, pk):
    try:
        ...
        if request.method == 'POST':
            fed.delete()
            messages.success(request, "Tarqatish muvaffaqiyatli oʻchirildi")
            return redirect('pharmacist_prescription')

    except:
        messages.error(request, "O'chirish xatosi, yana tekshiring")
        return redirect('pharmacist_prescription')
