...
class PatientForm(forms.Form):
    ...
    first_name = forms.CharField(label="Ism", max_length=50, widget=forms.TextInput(attrs={"class":"form-control"}))
    last_name = forms.CharField(label="Familya", max_length=50, widget=forms.TextInput(attrs={"class":"form-control"}))
    address = forms.CharField(label="Manzil", max_length=50, widget=forms.TextInput(attrs={"class":"form-control"}))
    phone_number = forms.CharField(label="Tel", max_length=50)
    gender_list = (
        ('Male','Erkak'),
        ('Female','Ayol')
    )
    gender = forms.ChoiceField(label="Jins", choices=gender_list, widget=forms.Select(attrs={"class":"form-control"}))
    ...
    def clean_reg_no(self):
        ...
        if  not  reg_no:
            raise ValidationError("Ushbu qator to'ldirilishi shart")
        for instance in Patients.objects.all():
            if instance.reg_no==reg_no:
                raise ValidationError("Ro‘yxatga olish raqami allaqachon mavjud")
    ...
    def clean_phone_number(self):
        ...
        if not phone_number:
            raise forms.ValidationError('Ushbu qator to\'ldirilishi shart')
        elif len(phone_number) < 10:
            raise forms.ValidationError('Invalid Number')
        for instance in Patients.objects.all():
            if instance.phone_number==phone_number:
                raise ValidationError( "Telefon raqami allaqachon mavjud")
    ...
    def clean_username(self):
        ...
        for instance in CustomUser.objects.all():
            if instance.username==username:
                raise ValidationError( "Foydalanuvchi usernami allaqachon mavjud")
...
class EditPatientForm(forms.Form):
    ...
    last_name = forms.CharField(label="Familya", max_length=50, widget=forms.TextInput(attrs={"class":"form-control"}))
    address = forms.CharField(label="Manzil", max_length=50, widget=forms.TextInput(attrs={"class":"form-control"}))
    phone_number = forms.CharField(label="Tel", max_length=50, widget=forms.TextInput(attrs={"class":"form-control"}))
    gender_list = (
        ('Male','Erkak'),
        ('Female','Ayol')
    )
...
class StockForm(forms.ModelForm):
    valid_to= forms.DateField(label="Quyidagi sanagacha foydalanilsin", widget=DateInput(attrs={"class":"form-control"}))
