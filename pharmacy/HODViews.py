...
def createPatient(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            user.save()
            messages.success(request, username +'Muvaffaqiyatli qo\'shildi')
...
def confirmDelete(request,pk):
    try:
        ...
        messages.error(request, "Bemorni oʻchirib boʻlmaydi. Bemor hali ham dori-darmonlarni qabul qilmoqda yoki xatolik yuz berdi")
        return redirect('all_patients')
...
def createPharmacist(request):
    ...
    if request.method == "POST":
        ...
        user.save()
        messages.success(request, "Xodimlar muvaffaqiyatli qo‘shildi!")
...
def createDoctor(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(request, "Xodimlarni qo‘shib bo‘lmadi!")
            return redirect('add_doctor')
...
def addCategory(request):
    try:
        ...
        if request.method == 'POST':
            if form.is_valid():
                form.save()
                messages.success(request, "Turkum muvaffaqiyatli qo'shildi!")

                return redirect('add_category')
    except:
        messages.error(request, "Turkum qo'shilmagan! Qayta urinib ko'ring")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # First Update into Custom User Model
                ...
                patients_edit.save()
                messages.success(request, "Bemor muvaffaqiyatli yangilandi!")
                return redirect('all_patients')
            except:
                messages.success(request, "Bemorni yangilab boʻlmadi.")
                return redirect('all_patients')
...
def deleteDoctor(request,pk):
    try:
        ...
        if request.method == 'POST':
            doctor.delete()
            messages.success(request, "Doktor muvaffaqiyatli oʻchirildi")

            return redirect('manage_doctor')

    except:
        messages.error(request, "Doktor allaqachon o'chirilgan")
        return redirect('manage_doctor')
...
def deletePharmacist(request,pk):
    try:
        ...
        if request.method == 'POST':
            pharmacist.delete()
            messages.success(request, "Farmatsevt muvaffaqiyatli oʻchirildi")
                
            return redirect('manage_pharmacist')

    except:
        messages.error(request, "Farmatsevt o'chirildi")
        return redirect('manage_pharmacist')
...
def deletePharmacyClerk(request,pk):
    try:
        ...
        if request.method == 'POST':
            ...
            clerk.delete()
            messages.success(request, "Dorixona xodimi muvaffaqiyatli oʻchirildi")
                
            return redirect('manage_pharmacyClerk')

    except:
        messages.error(request, "Dorixona xodimi o'chirilmagan")
        return redirect('manage_pharmacyClerk')
...
def editPharmacist(request,staff_id):
    ...
    if request.method == "POST":
        ...
        messages.success(request, "Xodimlar muvaffaqiyatli yangilandi.")
        return redirect('manage_pharmacist')
    context = {
        "staff": staff,
        "id": staff_id,
...
def editPharmacyClerk(request,clerk_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.success(request,'Qabulxona xodimi muvaffaqiyatli yangilandi')
        except:
            messages.success(request,'Qabulxonada xatolik yuz berdi yangilanmagan')


        
    context={
        "staff":clerk,
        "title":"PharmacyClerk-ni tahrirlash"
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            try:
                ...
                messages.error(request,'Qabulxonada xatolik yuz berdi yangilanmagan')
...
def deleteDrug(request,pk):
    try:
        ...
        messages.error(request, "Farmatsevt allaqachon o'chirilgan")
        return redirect('manage_stock')
...
def receiveDrug(request,pk):
    ...
    try:
        ...
        if form.is_valid():
            ...
            messages.success(request, str(instance.receive_quantity) + " " + instance.drug_name +" "+ "dorilar muvaffaqiyatli qo'shildi")

            return redirect('manage_stock')

      
    except:
        messages.error(request,"Xatolik yuz berdi, dori miqdori qo'shilmadi")
...
def reorder_level(request, pk):
    ...
    if form.is_valid():
        instance = form.save(commit=False)
        instance.save()
        messages.success(request, "Qayta tartiblash uchun " + str(instance.drug_name) + " ga yangilanadi " + str(instance.reorder_level))
