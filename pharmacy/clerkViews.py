...
def createPatient(request):
    ...
    try:
        ...
        messages.error(request,'Bemor saqlanmadi')
        return redirect('patient_form2')
    context={
        "form":form
    }
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    messages.error(request, "Bemorni yangilab bo‘lmadi.")
                    return redirect('all_patients2')
    except:
         messages.error(request, "Yaroqsiz xato!")
         return redirect('all_patients')
...
def confirmDelete(request,pk):
    try:
        ...
        if request.method == 'POST':
            patient.delete()
            messages.success(request, "Xodimlar oʻchirildi")
